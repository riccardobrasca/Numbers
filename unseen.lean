import Lean
import Numbers.Nopush.Naturals_nosorry
...
def printDeps₂ (k : Name) (v : Array Name) (print : String → IO Unit) : IO Unit := do
  for val in v do
    if (`Numbers).isPrefixOf val then
...
def groups (imports : NameMap (Array Name)) : NameMap Unit :=
  RBMap.fromList (imports.fold (fun xs k _ =>
    if (`Numbers).isPrefixOf k then (group k, ()) :: xs else xs) []) _
...
elab "#deptree " : command => do
  ...
  for (gp, _) in groups imports do
    ...
    for (k, v) in imports do
      if (`Numbers).isPrefixOf k && group k = gp then do
    ...
    h.write "}\n".toUTF8
  for (k, v) in imports do
    if (`Numbers).isPrefixOf k then do
      printDeps₂ k v (fun s => h.write s.toUTF8)
  h.write "}\n".toUTF8
...
def allFiles (env : Environment) : List Name :=
  (env.importGraph.fold (fun xs k _ => if (`Numbers).isPrefixOf k then
