...
\begin{frame}{A positive rational below a positive real}
  ...
  \(0<k(\varepsilon)<E\), then one can find \(r\in\Q\) with
  \[
    \boxed{|X-k(r)|<E.}
  \]
\end{frame}
...
\begin{frame}{The idea}
  ...
  \]
  Each \(k(x_n)\) is represented by the constant row obtained by
  \emph{freezing} \(x_n\), whereas \(X\) is represented by the moving bottom
  row:
  \[
    \begin{array}{c|ccccc}
      \text{real number}
  ...
  sequence, the resulting constant rows get closer to the moving bottom row.
\end{frame}

\begin{frame}{Why the idea works}
  \small
  If \(N\) works for the Cauchy condition at tolerance \(\varepsilon\), then
  take any \(n\ge N\).  In every sufficiently late column \(m\ge N\), the
  entries in the \(k(x_n)\)-row and the \(X\)-row are respectively \(x_n\)
  and \(x_m\), so
  \[
    |x_n-x_m|\le\varepsilon
    \qquad(n,m\ge N).
  ...
  \begin{center}
    \(k(x_n)\)-row: \quad \(x_n,\ x_n,\ x_n,\ldots\)\\[3pt]
    \(X\)-row:      \quad \(x_0,\ x_1,\ x_2,\ldots\)
  \end{center}

  The rows may be unrelated in the finitely many initial columns.  That does
  not matter: the quotient order only asks for the bound \emph{eventually}.
  Hence
  \[
    |k(x_n)-X|\le k(\varepsilon).
  \]

  Since the tolerance \(\varepsilon>0\) can be arbitrarily small, the frozen
  rows converge to the moving row: \(k(x_n)\to X\).
\end{frame}
...
\begin{frame}{Approximating a real sequence term by term}
  ...
  We want to prove that \(F_n\) converges to the real number given by the pre-real \(x_n\).
...
\begin{frame}{Every real Cauchy sequence converges}
  ...
  \]
  For \(n\ge\max(N,M)\), the approximation bound and monotonicity of
  reciprocals give
  \begin{align*}
    |F_n-X|
