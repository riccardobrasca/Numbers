...
\begin{frame}{A Cauchy tail controls the real distance}
...
\begin{frame}{A rational approximation to any real}
  ...
  Cauchy property of \(x\) with the smaller tolerance \(\varepsilon/2\).
  Choose a corresponding threshold \(N\), and set
  \[
    r=x_N\in\Q.
  \]

  The preceding result applies directly:
  \[
    |X-k(r)|
      \le k\!\left(\frac\varepsilon2\right)
      < k(\varepsilon).
  \]

  Thus the single rational term \(r=x_N\), viewed inside \(\R\), lies within
  the prescribed distance of \(X\):
  \[
    \boxed{\exists r\in\Q,\qquad |X-k(r)|<k(\varepsilon).}
  \]
\end{frame}
