...
We construct the integers starting from the natural numbers $\MyNat$. Since \texttt{lean} already has a
...
\begin{definition}
    ...
    \lean{MyPreint}
    \leanok
    Let $\MyPreint$ be $\MyNat \times \MyNat$.
\end{definition}
...
\begin{proof}
    \leanok
    \uses{MyPreint.R}
    This follows by commutativity of addition in $\MyNat$.
\end{proof}
...
\begin{proof}
    ...
    a+d+c+f=b+c+d+e
    \]
    Since addition on $\MyNat$ is cancellative we get
    \[
    a+f = b + e
    \]
    as wanted.
...
\begin{definition}
    ...
    \leanok
    \uses{MyPreint.R}
We define an operation, called \emph{multiplication} on $\MyPreint$ as follows: the multiplication of $x = (a,b)$ and $y = (c, d)$ is
\[
...
\begin{proof}
    \uses{MyInt.add}
    \leanok
To prove the lemma it is enough to prove that, for all $a$, $b$, $c$, $d$, $e$ and $f$ in $\MyNat$, we have
\[
...
\]
that is true by associativity and commutativity in $\MyNat$.
\end{proof}
...
\begin{proof}
    ...
    so $0+1=0+0$ in $\MyNat$, that is absurd.
\end{proof}
...
\begin{proof}
    ...
    It is enough to prove that, for all $a$, $b$, $c$ and $d$ in $\MyNat$ such that $a \neq b$ and $c \neq d$ we have $a*c+b*d\neq a*d+b*c$. If this is not the case we must have $a = b$ or $c = d$. (Can you see how to prove this in $\MyNat$? You cannot use subtraction!) and we are done.
\end{proof}
...
\subsection{\texorpdfstring{The inclusion $i \colon \MyNat \to \MyInt$}{The inclusion}}
\begin{definition}
    \label{MyInt.i}
    \lean{MyInt.i}
    \uses{MyInt}
...
\begin{proof}
    \uses{MyInt.le, MyInt.i_zero, MyInt.i_injective}
    \leanok
    Let $x$ and $y$ be in $\MyInt$. We can write $x = ⟦ (a,b) ⟧$ and
    $y = ⟦ (c,d) ⟧$ and we need to prove that
    $⟦ (a,b) ⟧ \leq ⟦ (c,d) ⟧$ or $⟦ (c,d) ⟧ \leq ⟦ (a,b) ⟧$. Let's consider two cases (we use that the order on $\MyNat$ is total):
    \begin{itemize}
...
\begin{lemma}
    ...
    \lean{MyInt.i_le_iff}
    \leanok
    Given two natural numbers $a$ and $b$, we have $i(a) \leq i(b)$ if and only if $a \leq b$.
\end{lemma}
\begin{proof}
...
\begin{lemma}
    ...
    \leanok
    Let $x$, $y$ and $z$ in $\MyInt$ be such that $x \leq y$. Then $x + z \leq y + z$.
\end{lemma}
\begin{proof}
    \uses{MyInt.le, MyInt.add}
    \leanok
    Let $n$ be such that $y = x + i(n)$. It's immediate that $n$ also works to show that $x + z \leq y + z$.
\end{proof}
...
\begin{lemma}
    \label{MyInt.archimedean}
    \lean{MyInt.archimedean}
    \leanok
    \uses{MyInt.le, MyInt.i}
    For every $x$ in $\MyInt$ there exists $n$ in $\MyNat$ such that $x \leq i(n)$.
\end{lemma}
\begin{proof}
    \leanok
    Write $x = ⟦(a,b)⟧$. Then $i(a) = x + i(b)$ in $\MyInt$, so $x \leq i(a)$.
\end{proof}
