...
Everything starts with the natural numbers. Since \texttt{lean} already has a type called $\N$, we
define our own copy, called $\MyNat$, from scratch. To make sure we do not secretly use results that
are already in mathlib, the file defining $\MyNat$ does not import anything. From now on we will also
write $\N$ for $\MyNat$.
