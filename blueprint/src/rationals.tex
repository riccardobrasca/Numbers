...
\begin{definition}
    ...
    \leanok
    \uses{MyInt}
    Let $\MyPrerat$ be $\MyInt \times (\MyInt \setminus \{0\})$.
\end{definition}
...
\begin{definition}
    ...
    \leanok
    \uses{MyPrerat.R}
We define an operation, called \emph{multiplication} on $\MyPrerat$ as follows: the multiplication of $x = (a,b)$ and $y = (c, d)$ is
\[
x * y = (a,b) * (c,d) = (a*c, b*d)
\]
\end{definition}
...
\begin{definition}
    ...
    \leanok
    \uses{MyPrerat.R}
We define an operation, called \emph{inversion} on $\MyPrerat$ as follows: the inverse of $x = (a,b)$ is:
\[
...
\begin{definition}
    ...
    \leanok
We define the negation of $x = ⟦ (a, b) ⟧$ in $\MyRat$ as
\[
-x = ⟦ -(a, b) ⟧
\]
...
\begin{definition}
    ...
    \leanok
We define the addition of $x = ⟦ (a, b) ⟧$ and $y = ⟦ (c, d) ⟧$ in $\MyRat$ as
\[
x + y = ⟦ (a,b)+(c,d) ⟧
\]
...
\begin{definition}
    ...
    \leanok
We define the multiplication of $x = ⟦ (a, b) ⟧$ and $y = ⟦ (c, d) ⟧$ in $\MyRat$ as
\[
x * y = ⟦ (a, b)*(c,d) ⟧
\]
...
\begin{definition}
    ...
    \leanok
We define the inverse of $x = ⟦ (a, b) ⟧$ in $\MyRat$ as
\[
x^{-1} = ⟦ (a, b)^{-1} ⟧
\]
...
\subsection{\texorpdfstring{The inclusion $i \colon \MyNat \to \MyRat$}{The inclusion}}
\begin{definition}
    \label{MyRat.i}
    \lean{MyRat.i}
    \uses{MyRat}
...
\begin{lemma}
    ...
    \lean{MyRat.i_add}
    \leanok
For all $a$ and $b$ in $\MyNat$ we have that
\[
i(a+b) = i(a) + i(b)
\]
\end{lemma}
...
\begin{proof}
    \uses{MyRat.i}
    \leanok
    Let $a$ and $b$ be such that $i(a)=i(b)$. This means
    $⟦ (\MyInt.i a,1) ⟧ = ⟦ (\MyInt.i b,1) ⟧$, so
    $(\MyInt.i a) * 1 = 1 * (\MyInt.i b)$. Hence $\MyInt.i a = \MyInt.i b$, and injectivity
    of $\MyInt.i$ gives $a = b$.
\end{proof}
...
\begin{definition}
    ...
    \leanok
Given $x$ in $\MyRat$, we say that $x$ is \emph{nonnegative} if it has a representative with
nonnegative numerator and positive denominator: there exist $a$ and $b$ in $\MyInt$ with
$0 \leq a$ and $0 < b$ such that
\[
x = ⟦(a,b)⟧.
\]
...
\begin{lemma}
    ...
    \leanok
    Let $x$, $y$ and $z$ in $\MyRat$ be such that $x \leq y$. Then $x + z \leq y + z$.
\end{lemma}
\begin{proof}
...
\begin{lemma}
    ...
    \lean{MyRat.i_le_iff}
    \leanok
    Let $x$ and $y$ in $\MyNat$. We have that $i(x) \leq i(y)$ if and only if $x \leq y$.
\end{lemma}
\begin{proof}
...
\begin{lemma}
    \label{MyRat.archimedean}
    \lean{MyRat.archimedean}
    \leanok
    \uses{MyRat.i, MyRat.j_le_iff, MyInt.archimedean}
    For every $x$ in $\MyRat$ there exists $n$ in $\MyNat$ such that $x \leq i(n)$.
\end{lemma}
\begin{proof}
    \leanok
    Choose a representative $x = a/b$ with $0 < b$. By Lemma~\ref{MyInt.archimedean}, choose a
    natural number $n$ such that $a \leq \MyInt.i(n)$. Since $b$ is positive and $\MyInt.i(n)$ is
    nonnegative, we have $\MyInt.i(n) \leq b \cdot \MyInt.i(n)$. Thus
    $a \leq b \cdot \MyInt.i(n)$, which is exactly $a/b \leq n$, hence $x \leq i(n)$.
\end{proof}
