...
\begin{proof}
    \leanok
    \uses{MyPrereal.R}
    Exercice, using the triangle inequality and splitting $\varepsilon$ into $\varepsilon/2 +
    \varepsilon/2$.
\end{proof}
...
\begin{proof}
    \leanok
    Exercice, splitting $\varepsilon$ into $\varepsilon/2 + \varepsilon/2$.
\end{proof}
...
\begin{proof}
    \leanok
    Exercice, again using boundedness.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{MyPrereal.not_isPos_zero}
    If $x \not\approx 0$ then $x$ is positive, and then $-x$ cannot be nonnegative by
    Lemma~\ref{MyPrereal.not_isPos_zero}.
\end{proof}
...
Exercice, using that a positive real is the class of a positive prereal and
...
\begin{lemma}
    ...
    $i \colon \N \to \MyRat$ is the inclusion.
\end{lemma}
\begin{proof}
    \leanok
    Pick a rational $r$ with $x < k(r)$ by density, and use that $\N$ is archimedean in $\MyRat$.
\end{proof}
